...
Most of these operations are also possible with $\mathbf{MIN}_n(W)$ and $\mathbf{MOD}_m(W)$,
with caveats.
One example of these operations is \emph{downsampling}:
the contiguous value range for Scaled MinHash sketches allow deriving $\mathbf{SCALED}_{s'}(W)$ sketches for any $s' \gt s$ using only $\mathbf{SCALED}_{s}(W)$,
$\mathbf{MIN}_n(W)$ and $\mathbf{MOD}_m(W)$ also support this operation,
as long as $n' \lt n$ and $m'$ is a multiple of $m$.
