...
Once each sequence (from raw reads or assembled contigs) have a taxonomic assignment,
...
`gather` is an example of such top-down approach:
...
a $k$-mer aggregative method with explicit representation of datasets based on hierarchical indices and an specialization of the Sequence Bloom Tree [@solomon_fast_2016],
...
<!-- TODO: reproduce fig 6 in CAMI tutorial preprint -->

```{r gatherCAMImgSpider, eval=TRUE, echo=FALSE, message=FALSE, error=FALSE, warning=FALSE, cache=TRUE, out.width="100%", auto_pdf=TRUE, fig.cap="CAMI II Mouse Gut spider plot"}
knitr::include_graphics('figure/cami_ii_mg_recall_precision.pdf')
```
