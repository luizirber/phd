...
<!-- TODO: Fig 6 in CAMI tutorial preprint -->
...
\begin{algorithm}[ht]
   ...
   $Q' \leftarrow Q$\;
   \Loop {
       $best = \FindBestContainment(Q', C, T)$\;
       \If{$best = \emptyset$ }{
           break\;
       }
...
but each index is allowed to implement it in the most efficient or performant way:
  ...
  1. For `Linear` indices, the `FindBestContainment` operation is implemented as a linear scan over the list of signatures (as in Algorithm \@ref(alg:list));
