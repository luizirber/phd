...
but _mash screen_ again needs access to original dataset.
...
as well as support future use cases derived from the _Scaled MinHash_ functionalities.
...
due to multiple reasons:
  ...
  - since _Scaled MinHash_ sketches use a subset of the $k$-mer composition,
    they also scale better than full representations,
...
By considering methods like spaced $k$-mers [@leimeister_fast_2014] and minimizers [@roberts_reducing_2004] for computing the $k$-mer composition,
as well as alternative encodings for the nucleotides using 6-frame translation to amino acid [@gish_identification_1993],
Dayhoff [@dayhoff_model_1978] or other reduced alphabets [@peterson_reduced_2009],
comparisons on longer evolutionary distances can also improve taxonomic profiling by increasing the sensitivity of the containment estimation.
Fundamentally it also doesn't change the `gather` method,
