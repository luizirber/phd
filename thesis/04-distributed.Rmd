...
with input sizes in the order of thousands of times larger than the final signature,
which lends itself to approaches where distributed workers do all the expensive data access,
and report back a small amount of data.
Systems like Folding@home [@beberg_folding_2009] focus on small amounts of input data
and heavy computational requirements instead,
but have similar architectures where a central server distributes tasks to
workers and collect results.
...
with the European Nucleotide Archive (ENA) [@leinonen_european_2011] and the DNA Database of Japan (DDBJ) [@kaminuma_ddbj_2010] being other members.
...
The Integrated Microbial Genomes and Microbiomes (IMG/M) [@chen_imgm_2019] contains assembled genomes and metagenomes for sequencing done at the Department of Energy's Joint Genome Institute,
