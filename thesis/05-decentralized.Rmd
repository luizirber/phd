...
```{=latex}
\epigraph {A complex system that works is invariably found to have evolved from a
...
Chapter 4 introduced wort,
<!-- TODO internal link to chapter 4 -->
...
<!-- TODO: link indices to chp2, and signatures to chp1? -->
...
equivalent to the yearly energy consumption of whole countries [@digiconomist_bitcoin_nodate].
...
At the same time,
some tasks are better executed by centralization,
...
but while everyone wants high-quality data,
...
```{r sbtInsert, eval=TRUE, echo=FALSE, message=FALSE, error=FALSE, warning=FALSE, cache=TRUE, out.width="500px", fig.width=4, fig.cap="Inserting a dataset in an SBT"}
...
a full SBT index versus a leaf-only SBT index,
...
and all experiments are running a `sourmash gather` command on an SBT index with 5914 signatures.
